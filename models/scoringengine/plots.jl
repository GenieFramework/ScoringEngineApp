...
function one_way_data(df, groupvar, nbins = 10; targets, method = "quantiles")
    ...
    dfs = DataFrames.select(
        df,
        groupvar => (x -> binvar(x, nbins; method)) => "var_bins",
        targets,
    )
...
One-way plot
"""
const color_dict = Dict("event" => j_red, "flux" => j_green, "gbt" => j_purple)
...
function one_way_plot(df; targets)
    ...
    return (traces = traces, layout = p_layout, config = p_config)
end
